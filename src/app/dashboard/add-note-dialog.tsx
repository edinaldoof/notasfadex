...
// The server action will have its own schema to handle FormData.
const addNoteFormSchema = z.object({
  ...
  coordinatorEmail: z.string().email({ message: 'O e-mail do coordenador é inválido.' }),
...
export function AddNoteDialog({ open, onOpenChange, onNoteAdded }: AddNoteDialogProps) {
  ...
  const FormField = ({ id, label, icon: Icon, children, error }: { id: string, label: string, icon: React.ElementType, children: React.ReactNode, error?: string }) => (
                                ...
                                render={({ field }) => (
                                    <Popover>
                                        <PopoverTrigger asChild>
                                            ...
                                            >
                                                {field.value ? maskProjectAccount(field.value) : "Selecione ou digite a conta"}
                                        ...
                                        <PopoverContent className="w-[--radix-popover-trigger-width] p-0">
                                            <Command>
                                                <CommandInput 
                                                    placeholder="Buscar ou digitar conta..." 
                                                    value={field.value}
                                                    onValueChange={(currentValue) => field.onChange(maskProjectAccount(currentValue))}
                                                />
                                                <CommandList>
                                                    ...
                                                    <CommandGroup>
                                                        {existingAccounts.map((account) => (
                                                            <CommandItem
                                                                ...
                                                                value={account}
                                                                onSelect={() => {
                                                                    form.setValue('projectAccountNumber', account, { shouldValidate: true });
                                                                }}
                                                            >
                           ...
                           <Controller
                                ...
                                render={({ field }) => (
                                    <Input 
                                        {...field}
                                        id='prestadorCnpj' 
                                        placeholder='CNPJ de quem emitiu a nota' 
