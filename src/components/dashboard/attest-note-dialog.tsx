...
export function AttestNoteDialog({ open, onOpenChange, note, onConfirm, isPending }: AttestNoteDialogProps) {
  ...
  const [reportFile, setReportFile] = useState<File | undefined>();
  const [attestedFileName, setAttestedFileName] = useState<string | null>(null);
  const [reportFileName, setReportFileName] = useState<string | null>(null);
  const [observation, setObservation] = useState('');
  const [isPreviewOpen, setIsPreviewOpen] = useState(false); 

  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>, fileType: 'attested' | 'report') => {
    const file = event.target.files?.[0];
    if (file) {
      if (fileType === 'attested' && file.type !== 'application/pdf') {
        toast({
          title: 'Arquivo Inválido',
          description: 'Por favor, anexe um arquivo PDF para o atesto.',
          variant: 'destructive',
        });
        return;
      }

      if (fileType === 'attested') {
        setAttestedFile(file);
        setAttestedFileName(file.name);
      } else {
        setReportFile(file);
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="bg-slate-900 rounded-2xl border-slate-800/50 w-full max-w-lg">
          ...
          <div className="space-y-6 py-4">
              ...
              <div>
                  ...
                  <div className="relative border-2 border-dashed border-slate-700/50 rounded-xl p-6 text-center hover:border-primary/50 transition-colors">
                      ...
                      <p className="text-slate-400 mb-2">
                         {attestedFileName ? `Arquivo: ${attestedFileName}` : "Clique para fazer upload"}
                      </p>
                      ...
                      <Input 
                         ...
                         accept=".pdf" 
                         onChange={(e) => handleFileChange(e, 'attested')}
                         disabled={isPending}
                      />
                  </div>
              </div>
              ...
              <div>
                  <Label className="block text-sm font-medium text-slate-300 mb-2">Anexar Relatório de Execução (Opcional)</Label>
                  <div className="relative border-2 border-dashed border-slate-700/50 rounded-xl p-6 text-center hover:border-primary/50 transition-colors">
                      <Paperclip className="w-8 h-8 text-slate-400 mx-auto mb-2" />
                      <p className="text-slate-400 mb-2">
                         {reportFileName ? `Arquivo: ${reportFileName}` : "Clique para fazer upload"}
                      </p>
                      <Input 
                         type="file" 
                         className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" 
